 ...
 * @since 1.0.0
 */
add_action('widgets_init', 'pbc24com_register_posts_list_widget');

function pbc24com_register_posts_list_widget() {
    register_widget('Pbc24com_Posts_List');
}

class Pbc24com_Posts_List extends WP_widget {
    ...
    public function __construct() {
        $widget_ops = array(
            'classname' => 'pbc24com_posts_list',
            'description' => __('Display latest or random posts in list view.', 'pbc24com')
        );
        parent::__construct('pbc24com_posts_list', __('PBC24com: Posts Lists', 'pbc24com'), $widget_ops);
    }
    ...
    private function widget_fields() {
        ...
        $fields = array(
            ...
            'pbc24com_block_posts_count' => array(
                'pbc24com_widgets_name' => 'pbc24com_block_posts_count',
                'pbc24com_widgets_title' => __('No. of Posts', 'pbc24com'),
                'pbc24com_widgets_default' => 4,
                'pbc24com_widgets_field_type' => 'number'
            ),
            'pbc24com_block_posts_type' => array(
                'pbc24com_widgets_name' => 'pbc24com_block_posts_type',
    ...
    public function widget($args, $instance) {
        ...
        $pbc24com_block_posts_count = intval(empty($instance['pbc24com_block_posts_count']) ? 4 : $instance['pbc24com_block_posts_count']);
        $pbc24com_block_posts_type = empty($instance['pbc24com_block_posts_type']) ? '' : $instance['pbc24com_block_posts_type'];
        echo $before_widget;
        ?>
                        ...
                        <div class="single-post-wrapper clearfix">
                            <div class="post-thumb-wrapper">
                                <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
                                    <figure><?php the_post_thumbnail('pbc24com-block-thumb'); ?></figure>
                                </a>
                            </div>
                            <div class="post-content-wrapper">
                                <h3 class="post-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
                                <div class="post-meta-wrapper">
                                    <?php pbc24com_posted_on(); ?>
    ...
    public function form($instance) {
        ...
        <?php
        $widget_field = $widget_fields['pbc24com_block_posts_count'];
        extract($widget_field);
        ?>

        <p>
            <label for="<?php echo esc_attr($this->get_field_id($pbc24com_widgets_name)); ?>"><?php echo esc_html($pbc24com_widgets_title); ?>:</label><br />
            <input name="<?php echo esc_attr($this->get_field_id($pbc24com_widgets_name)); ?>" type="number" step="1" min="1" id="<?php echo esc_attr($this->get_field_name($pbc24com_widgets_name)); ?>" value="<?php echo esc_html($pbc24com_block_posts_count); ?>" />
        </p>

        <?php
        $widget_field = $widget_fields['pbc24com_block_posts_type'];
        extract($widget_field);
        ?>
        ...
        <div class="radio-wrapper">
            ...
            foreach ($pbc24com_widgets_field_options as $athm_option_name => $athm_option_title) {
                ?>
                <input id="<?php echo esc_attr($this->get_field_id($athm_option_name)); ?>" name="<?php echo esc_attr($this->get_field_name($pbc24com_widgets_name)); ?>" type="radio" value="<?php echo esc_html($athm_option_name); ?>" <?php checked($athm_option_name, $pbc24com_block_posts_type); ?> />
